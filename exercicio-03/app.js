...
brazilianFood[1] = "Pão de queijo"; //correct
...
const foodsInfo = `Ate aqui o array "BrazilianFoods" possui ${brazilianFood.length}: ${ brazilianFood[0]}, ${ brazilianFood[1]} e ${ brazilianFood[2]}`; // correct
...
const dogNames = maleDogNames.concat(femaleDogNames) // Lembrar que concat junta 2 arrays
...
const listDogNames = dogNames.join(', '); // correct
...
// dogNames.pop() // correct
// console.log(dogNames[dogNames.length - 1])  // passing the lenght inside brackets
...
dogNames.push('Nina'); // correct
// const initials = `${dogNames[0].slice(0,2).toUpperCase()}, ${dogNames[1].slice(0,2).toUpperCase()}, ${dogNames[2].slice(0,2).toUpperCase()}, ${dogNames[3].slice(0,2).toUpperCase()}`; 
// const initials = `${dogNames[0][0]}${dogNames[0][1]} ${dogNames[1][0]}${dogNames[1][1]} ${dogNames[2][0]}${dogNames[2][1]} ${dogNames[3][0]}${dogNames[3][1]}`; 
// is better use bracjets
...
// console.log(initials.toLowerCase()); // correct
...
const desert = `${dogNames[0].slice(3,5)}${dogNames[1].slice(2,5)}${dogNames[3].slice(2,5).replace('n', 'd')}`;
console.log(desert);
